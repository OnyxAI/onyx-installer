...
{
    ui->setupUi(this);
    utils::writeLog("Miror url : " + mirrorURL);
...
{
    bool hasONYXURL = false;
    if (reply->error() == QNetworkReply::NoError)
    {
        while (reply->canReadLine())
            if (enumerateBuilds(reply->readLine()))
                hasONYXURL = true; /* Check if we get at least one build back. We may get 200 but not the expected page.. */
        reply->deleteLater();
    }
    else if (reply->error() != QNetworkReply::NoError || ! hasONYXURL)
        utils::displayError(tr("Error connecting to network"), tr("<html><body>There seems to be an issue connecting to the network") + "\n" + "\n" + tr("Please check that you are not trying to download Onyx via a proxy server</body></html>"), false);
}
...
void VersionSelection::on_versionnextButton_clicked()
{
    ...
    {
        utils::displayError(tr("Select a version"), tr("<html><body>Please select a version!</body></html>"));
...
void VersionSelection::on_useLocalBuildCheckbox_stateChanged(int checkState)
{
    if (checkState == Qt::Checked)
    {
        ...
        buildName = QFileDialog::getOpenFileName(this, "Select disk image", dir, tr("Onyx Disk Images (**.img.gz)"));
        if (buildName == NULL)
        {
            utils::displayError(tr("Build selection error"), tr("<html><body>You didn't select a custom build -- reverting to online build</body></html>s"));
