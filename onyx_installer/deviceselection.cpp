...
void DeviceSelection::on_devicenextButton_clicked()
{
    ...
    if (checkCount == 0)
    {
        utils::displayError(tr("Please select a device"), tr("<html><body>You must select one device to image</body></html>"));
        ui->devicenextButton->setEnabled(true);
        ui->refreshButton->setEnabled(true);
    } else if (checkCount > 1)
    {
        utils::displayError(tr("Please select one device"), tr("<html><body>You can only select one device to image</body></html>"));
        ui->devicenextButton->setEnabled(true);
        ui->refreshButton->setEnabled(true);
    } else {
